...
static char *derive_module_name_from_aliases(const char *filename, const AliasVec *aliases)
{
    ...
    char *abs_file = get_full_path(filename);
    if (!abs_file)
        return NULL;
    ...
    {
        char *abs_dir = get_full_path(aliases->dirs[i]);
        if (!abs_dir)
            continue;
...
{
    char *resolved = get_full_path(start_path);
    if (resolved)
    {
        struct stat st;
...
#define APUSH(v, n, d)                                               \
    ...
    for (int i = 0; i < inc.count; i++)
        module_manager_add_search_path(&an.module_manager, inc.items[i]);
    for (int i = 0; i < al.count; i++)
        module_manager_add_alias(&an.module_manager, al.names[i], al.dirs[i]);
    ...
    {
        if (an.errors.count > 0)
        {
            fprintf(stderr, "semantic analysis failed with %d error(s):\n", an.errors.count);
            semantic_print_errors(&an, &lx, filename);
        }
        if (an.module_manager.had_error)
        {
            fprintf(stderr, "module loading failed with %d error(s):\n", an.module_manager.errors.count);
            module_error_list_print(&an.module_manager.errors);
        }
        if (cfg)
        {
            config_dnit(cfg);
    ...
    if (!codegen_generate(&cg, prog, &an))
    {
    ...
    if (cfg)
    {
        ...
        if (config_has_runtime_module(cfg))
        {
            ...
            if (!m)
                m = module_manager_load_module(&an.module_manager, rt);
            if (m)
            {
                m->needs_linking = true;
